...
git clone [--template=<template-directory>]
	  ...
	  [--filter=<filter-spec> [--also-filter-submodules]] [--bot] [--] <repository>
	  [<directory>]
...
`--bot`::
	Optimize for automated/bot use: suppress all output (implies `--quiet`)
	and disable all interactive credential prompts by setting
	`GIT_TERMINAL_PROMPT=0`, `GIT_ASKPASS`, and `SSH_ASKPASS` to empty
	values. This is useful for CI/CD pipelines and scripts where no user
